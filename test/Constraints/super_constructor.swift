...
class D : B {
  ...
  init(g:Int) {
    ...
    // expected-note @-1 {{overloads for 'B.init' exist with these partially matching parameter lists: (x: Int), (a: Unicode.Scalar), (b: Unicode.Scalar), (z: Float)}}
  }
...
class B {
  ...
  init(a:Unicode.Scalar) {
  }
  init(b:Unicode.Scalar) {
  }
